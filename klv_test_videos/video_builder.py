...
class KLVMetadataGenerator:
    ...
    def create_packet_from_dict(self, metadata: Dict[str, Any]) -> bytes:
        ...
        if 'sensor_relative_azimuth' in metadata:
            elements.append(misb0601.SensorRelativeAzimuthAngle(
                metadata['sensor_relative_azimuth']
            ))

        if 'sensor_relative_elevation' in metadata:
            elements.append(misb0601.SensorRelativeElevationAngle(
...
class VideoFrameGenerator:
    ...
    def generate_frame(self, frame_num: int, total_frames: int,
        ...
        cv2.circle(frame, (self.width-5, self.height-5), marker_size,
                  (255, 255, 0), -1)  # Bottom-right
...
def build_klv_video(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Mux with FFmpeg
        print(f"Muxing video and KLV with FFmpeg...")
        ...
        ffmpeg_cmd = [
            ...
            '-g', str(fps),  # Keyframe interval (every second)
            '-x264-params', 'bframes=0:keyint={}:scenecut=0'.format(fps),  # No B-frames
            '-bsf:v', 'h264_mp4toannexb',  # Convert to Annex-B for MPEG-TS
            '-muxdelay', '0',  # No muxing delay
            '-muxpreload', '0',  # No muxing preload
            '-c:d', 'copy',  # Copy data stream
            '-metadata:s:d:0', 'language=klv',  # Try to hint KLV
            '-f', 'mpegts',  # MPEG-TS format (same as sample_video.mpg)
            output_path
        ]
        ...
        if result.returncode == 0:
            ...
            print(f"      For KWIVER testing, use the separate .klv file.")
            print(f"      (FFmpeg cannot fully replicate KLVA codec tagging)")

        success = result.returncode == 0
        if not success:
            print(f"Warning: FFmpeg muxing may have issues.")
